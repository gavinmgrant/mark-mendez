  ...
  block: {
    ...
    h2: ({ children, value }) => {
      ...
      return (
        <h2
          id={slug}
          className="scroll-m-20 border-b pb-2 text-2xl font-semibold first:mt-0"
        >
          {children}
        </h2>
      );
...
export function RichText<T>({
  ...
  return (
    <div
      className={cn(
        "prose prose-slate prose-headings:scroll-m-24 prose-headings:font-bold prose-headings:text-opacity-90 prose-p:text-opacity-80 prose-a:underline prose-a:decoration-dotted prose-ol:list-decimal prose-ol:text-opacity-80 prose-ul:list-disc prose-ul:text-opacity-80 prose-h2:border-b prose-h2:pb-2 prose-h2:text-2xl prose-h2:font-semibold prose-h2:prose-h2:first:mt-0 max-w-none dark:prose-invert",
        className,
      )}
    >
      <PortableText
